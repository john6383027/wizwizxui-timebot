...
if (preg_match('/^createAccAmount(\d+)_(\d+)_(\d+)/', $userInfo['step'], $match) && $text != $buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)) {
    ...
    for ($i = 1; $i <= $text; $i++) {
        ...
        foreach ($vraylink as $link) {
            $subLink = explode(":", $subLink);
            $subLink = "https://portal.datacamp.work:" . $subLink[2];
            $acc_text = "
...
if (preg_match('/selectServer(?<serverId>\d+)_(?<buyType>\w+)/', $data, $match) && ($botState['sellState'] == "on" || ($from_id == $admin || $userInfo['isAdmin'] == true))) {
    ...
    if ($respd->num_rows == 0) {
        ...
        $keyboard[] = ['text' => $buttonValues['back_to_main'], 'callback_data' => ($match['buyType'] == "one" ? "agentOneBuy" : ($match['buyType'] == "much" ? "agentMuchBuy" : "buySubscription"))];
        $keyboard = array_chunk($keyboard, 1);
...
if ($data == "managePanel" and (($from_id == $admin || $userInfo['isAdmin'] == true))) {
    ...
    setUser();
    $msg = "به بخش مدیریت خوش امدید، با استفاده از دکمه های زیر میتوانید ربات را مدیریت کنید.
...
if ($data == 'reciveApplications') {
    ...
    $keyboard[] = ['text' => $buttonValues['back_to_main'], 'callback_data' => "mainMenu"];
    $keyboard = array_chunk($keyboard, 1);
    editText($message_id, "📌 با توجه به سیستم عامل خود بر روی دکمه های شیشه ای کلیک کرده و اقدام به نصب نرم افزار های مورد نیاز کنید. در نهایت با توجه به متن راهنمای زیر میتوانید از لینک اشتراک های خریداری شده استفاده نمایید.

💎  اندروید ، نرم افزار v2rayng :
بعد از نصب v2rayng ، لینک اشتراک خود را کپی کنید. لینک های اشتراک معمولا با کلمه vmess شروع می‌شوند.
پس از کپی کردن لینک ، اپلیکیشن را باز کرده و گزینه + را بزنید. سپس از گزینه های باز شده بر روی گزینه import from clipboard کلیک کنید.
در صورتی که میخواهید لینک را وارد گوشی دیگری که تلگرام بر روی آن نصب نیست، اضافه کنید، میتوانید qr code اشتراک را از طریق گزینه import config from QrCode در داخل اپلیکیشن اسکن نمایید.


💎 ایفون و ایپد ، اپلیکیشن streisand : 
پس از نصب اپلیکیشن از اپ استور، لینک اشتراک را کپی کرده ، و داخل اپلیکیشن گزینه + را بزنید ، سپس بر روی گزینه import from clipboard کلیک کنید تا لینک اتصال به اپلیکیشن اضافه شود.
همچنین برای سهولت اضافه کردن لینک در داخل گوشی های دیگر ، میتوانید از طریق اسکن QrCode در داخل خود اپلیکیشن، لینک اتصال را اضافه کنید.


💎 ویندوز ، لینوکس ، نرم افزار Nekoray :
ابتدا نرم افزار را دانلود کنید و از حالت فشرده خارج کنید و وارد نرم افزار شوید ، سپس لینک را کپی کنید و داخل نرم افزار ctrl+v بزنید تا لینک اتصال وارد نرم افزار شود.
گزینه tun mode یا vpn mode را تیک بزنید تا فعال شود.
روی سرور وارد شده کلیک راست کرده و start را بزنید.


💎 مک ، نرم‌افزار hiddify :
ابتدا نرم افزار را از اپ استور نصب کنید. سپس لینک اتصال را کپی کرده ،  داخل نرم افزار ctrl+v بزنید تا لینک اتصال وارد نرم افزار شود.

💡 نکات مهم : 
لطفا توجه داشته باشید که حتما ساعت و تاریخ دستگاه روی حالت تنظیم خودکار باشد.

📱 در دستگاه های اندروید و IOS (ایفون و آیپد ) میتوانید از بخش تنظیمات ساعت و تاریخ ، تیک گزینه تنظیم اتوماتیک را بزنید.

💻 در دستگاه های دسکتاپ هم می‌توانید از بخش تنظیمات ساعت و تاریخ گزینه تنظیم اتوماتیک را بزنید. اما ممکن است با این مورد ساعت و تاریخ به صورت خودکار تنظیم نشود که نیاز است به صورت دستی اقدام به تنظیم دقیق اقدام بفرمایید.

🕘 و در نهایت حتما چک شود که تایم زون دستگاه روی ایران/تهران باشد.", json_encode(['inline_keyboard' => $keyboard]));
}
